...
["_interceptors", "dart:_interceptors", , J, {
  ...
  Interceptor: {
    ...
    "%": "DOMError|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|SVGAnimatedString|SVGRect|WebGLBuffer|WebGLProgram|WebGLShader|WebGLUniformLocation"
  },
  JSBool: {
    "^": "Interceptor;",
    toString$0: function(receiver) {
...
["glGame", "webgl.dart", , R, {
  "^": "",
  getVertexData: function(vertices) {
    var data, i;
    data = H.setRuntimeTypeInfo([], [P.$double]);
    for (i = 0; i < vertices.length; ++i) {
      data.push(vertices[i].pos.storage[0]);
      if (i >= vertices.length)
        return H.ioore(vertices, i);
      data.push(vertices[i].pos.storage[1]);
      if (i >= vertices.length)
        return H.ioore(vertices, i);
      data.push(vertices[i].pos.storage[2]);
    }
    return data;
  },
  ...
  }, "call$1", "onKeyDown$closure", 2, 0, 7],
  onKeyUp: [function(e) {
    var t1, t2;
    t1 = J.get$keyCode$x(e);
    t2 = $.get$keys();
  ...
  }, "call$1", "onKeyUp$closure", 2, 0, 7],
  main: [function() {
    var t1, t2, height, width, left, bottom, right_minus_left, top_minus_bottom, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15, t16, t17, vertexShader, fragmentShader;
    t1 = document.querySelector("glCanvas");
    $.canvas = t1;
    if (t1 == null) {
      t1 = document.querySelector("#glCanvas");
      $.canvas = t1;
    }
    t1 = J.getContext$1$x(t1, "webgl");
    $.gl = t1;
    if (t1 == null) {
      t1 = J.getContext$1$x($.canvas, "experimental-webgl");
      $.gl = t1;
    }
    if (t1 == null)
      window.alert("WebGL is not supported in your browser");
    else {
      $.mesh = R.Mesh$fromFile("meshes/cube.obj");
      ...
      if (typeof t2 !== "number")
        return H.iae(t2);
      height = Math.tan(0.6108652381980153) * 0.1;
      width = height * (t1 / t2);
      left = -width;
      bottom = -height;
      ...
      t1[14] = 0;
      t1[15] = 0;
      t1[0] = 0.2 / right_minus_left;
      t1[5] = 0.2 / top_minus_bottom;
      t1[8] = (width + left) / right_minus_left;
      t1[9] = (height + bottom) / top_minus_bottom;
      t1[10] = -1.0002000200020003;
      t1[11] = -1;
      t1[14] = -0.20002000200020004;
      $.projectionMatrix = new T.Matrix4(t1);
      t1 = new Float32Array(16);
      t2 = new T.Matrix4(t1);
      t2.setIdentity$0();
      ...
      t16 = t1[11];
      t17 = t1[15];
      t1[12] = t2 * 0 + t3 * 0 + t4 * -5 + t5;
      t1[13] = t6 * 0 + t7 * 0 + t8 * -5 + t9;
      t1[14] = t10 * 0 + t11 * 0 + t12 * -5 + t13;
      t1[15] = t14 * 0 + t15 * 0 + t16 * -5 + t17;
      vertexShader = J.createShader$1$x($.gl, 35633);
      J.shaderSource$2$x($.gl, vertexShader, "attribute vec3 position;\r\n\r\nuniform mat4 projection;\r\nuniform mat4 modelView;\r\n\r\nvarying vec4 vColor;\r\n\r\nvoid main() {\r\n\tvColor = vec4(clamp(position, 0.0, 1.0), 1.0);\r\n\tgl_Position = projection * modelView * vec4(position, 1.0);\r\n}\r\n");
      J.compileShader$1$x($.gl, vertexShader);
      if (J.getShaderParameter$2$x($.gl, vertexShader, 35713) === 0)
        P.print(J.getShaderInfoLog$1$x($.gl, vertexShader));
      fragmentShader = J.createShader$1$x($.gl, 35632);
      J.shaderSource$2$x($.gl, fragmentShader, "precision mediump float;\r\n\r\nvarying vec4 vColor;\r\n\r\nvoid main() {\r\n\tgl_FragColor = vColor;\r\n}\r\n");
      J.compileShader$1$x($.gl, fragmentShader);
      if (J.getShaderParameter$2$x($.gl, fragmentShader, 35713) === 0)
        P.print(J.getShaderInfoLog$1$x($.gl, fragmentShader));
      t1 = J.createProgram$0$x($.gl);
      $.program = t1;
      J.attachShader$2$x($.gl, t1, vertexShader);
      J.attachShader$2$x($.gl, $.program, fragmentShader);
      J.linkProgram$1$x($.gl, $.program);
      if (J.getProgramParameter$2$x($.gl, $.program, 35714) === 0)
        P.print(J.getProgramInfoLog$1$x($.gl, $.program));
      J.validateProgram$1$x($.gl, $.program);
      if (J.getProgramParameter$2$x($.gl, $.program, 35715) === 0)
        P.print(J.getProgramInfoLog$1$x($.gl, $.program));
      J.useProgram$1$x($.gl, $.program);
      $.positionAttribute = J.getAttribLocation$2$x($.gl, $.program, "position");
      $.projectionUniform = J.getUniformLocation$2$x($.gl, $.program, "projection");
      $.modelViewUniform = J.getUniformLocation$2$x($.gl, $.program, "modelView");
      t1 = $.get$keys();
      t1.toString;
      if (typeof t1 !== "object" || t1 === null || !!t1.immutable$list)
        H.throwExpression(P.UnsupportedError$("fill range"));
      H.IterableMixinWorkaround_fillRangeList(t1, 0, 256, false);
      t1 = $.get$pkeys();
      t1.toString;
      if (typeof t1 !== "object" || t1 === null || !!t1.immutable$list)
        H.throwExpression(P.UnsupportedError$("fill range"));
      ...
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(R.onKeyDown$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
      t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_keyup._eventType, false), [null]);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(R.onKeyUp$closure()), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
      J.clearColor$4$x($.gl, 0, 0, 0, 1);
      J.viewport$4$x($.gl, 0, 0, J.get$width$x($.canvas), J.get$height$x($.canvas));
      J.frontFace$1$x($.gl, 2305);
      J.cullFace$1$x($.gl, 1029);
      J.enable$1$x($.gl, 2884);
      J.enable$1$x($.gl, 2929);
      R.run();
    }
  }, "call$0", "main$closure", 0, 0, 2],
  run: [function() {
    var t1, t2, t3, cosAngle, sinAngle, t4, t5, t6, t7, t8, t9, t10, tempList;
    t1 = Date.now();
    ...
    if (typeof t3 !== "number")
      return H.iae(t3);
    t3 = t2 + (t1 - t3) / 16.666666666666668;
    $.lag = t3;
    $.timer = t1;
    for (t1 = t3; t1 > 1;) {
      t1 = $.get$pkeys();
      ...
      t1 = $.transformationMatrix;
      t1.toString;
      cosAngle = Math.cos(0.01);
      sinAngle = Math.sin(0.01);
      t1 = t1.storage;
      t2 = t1[4];
      t3 = t1[8];
      t4 = t1[5];
      ...
      tempList = new Float32Array(16);
      $.projectionMatrix.copyIntoArray$1(tempList);
      J.uniformMatrix4fv$3$x($.gl, $.projectionUniform, false, tempList);
      $.transformationMatrix.copyIntoArray$1(tempList);
      J.uniformMatrix4fv$3$x($.gl, $.modelViewUniform, false, tempList);
      t1 = $.lag - 1;
      $.lag = t1;
    }
    t1 = window;
    C.Window_methods._ensureRequestAnimationFrame$0(t1);
    C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(R.render$closure()));
    P.Timer_Timer(C.Duration_0, R.run$closure());
  }, "call$0", "run$closure", 0, 0, 2],
  render: [function(time) {
    var t1;
    J.clear$1$ax($.gl, 16640);
    t1 = $.mesh;
    t1.toString;
    J.enableVertexAttribArray$1$x($.gl, $.positionAttribute);
    J.bindBuffer$2$x($.gl, 34962, t1.vbo);
    J.vertexAttribPointer$6$x($.gl, $.positionAttribute, 3, 5126, false, 0, 0);
    J.bindBuffer$2$x($.gl, 34963, t1.ibo);
    J.drawElements$4$x($.gl, 4, t1.size, 5123, 0);
    J.disableVertexAttribArray$1$x($.gl, $.positionAttribute);
  }, "call$1", "render$closure", 2, 0, 8],
  Mesh: {
    "^": "Object;vbo,ibo,size",
    Mesh$fromFile$1: function(path) {
      var t1, t2, t3;
      if (!C.JSString_methods.endsWith$1(path, ".obj")) {
        P.print("Invalid file format!");
        return;
      }
      this.vbo = J.createBuffer$0$x($.gl);
      this.ibo = J.createBuffer$0$x($.gl);
      this.size = 0;
      t1 = W.HttpRequest_getString(path, null, null).then$1(new R.Mesh$fromFile_closure(this));
      t2 = $.Zone__current;
      t3 = P._registerErrorHandler(new R.Mesh$fromFile_closure0(), t2);
      t2.toString;
      t1._addListener$1(H.setRuntimeTypeInfo(new P._Future(0, t2, null, null, null, null, t3, null), [null]));
    },
    static: {Mesh$fromFile: function(path) {
        var t1 = new R.Mesh(null, null, null);
        t1.Mesh$fromFile$1(path);
        return t1;
      }}
  },
  Mesh$fromFile_closure: {
    "^": "Closure:11;this_0",
    call$1: function(src) {
      var vertices, indices, lines, i, tokens, t1, t2, t3, t4;
      vertices = H.setRuntimeTypeInfo([], [R.Vertex]);
      indices = H.setRuntimeTypeInfo([], [P.$int]);
      lines = J.split$1$s(src, "\n");
      for (i = 0; i < lines.length; ++i) {
        tokens = J.split$1$s(lines[i], " ");
        t1 = tokens.length;
        if (t1 !== 0) {
          if (0 >= t1)
        ...
        if (t1)
          ...
          if (J.$eq(tokens[0], "v")) {
            ...
            t1 = H.Primitives_parseDouble(tokens[1], null);
            if (2 >= tokens.length)
              return H.ioore(tokens, 2);
            t2 = H.Primitives_parseDouble(tokens[2], null);
            if (3 >= tokens.length)
              return H.ioore(tokens, 3);
            t3 = H.Primitives_parseDouble(tokens[3], null);
            t4 = new Float32Array(3);
            t4[0] = t1;
            t4[1] = t2;
            t4[2] = t3;
            vertices.push(new R.Vertex(new T.Vector3(t4)));
          } else {
            if (0 >= tokens.length)
              return H.ioore(tokens, 0);
            if (J.$eq(tokens[0], "f")) {
              if (1 >= tokens.length)
                return H.ioore(tokens, 1);
              indices.push(J.$sub$n(H.Primitives_parseInt(tokens[1], null, null), 1));
              if (2 >= tokens.length)
                return H.ioore(tokens, 2);
              indices.push(J.$sub$n(H.Primitives_parseInt(tokens[2], null, null), 1));
              if (3 >= tokens.length)
                return H.ioore(tokens, 3);
              indices.push(J.$sub$n(H.Primitives_parseInt(tokens[3], null, null), 1));
            }
          }
        }
      }
      t1 = this.this_0;
      t1.size = indices.length;
      J.bindBuffer$2$x($.gl, 34962, t1.vbo);
      J.bufferDataTyped$3$x($.gl, 34962, new Float32Array(H._ensureNativeList(R.getVertexData(vertices))), 35044);
      J.bindBuffer$2$x($.gl, 34963, t1.ibo);
      J.bufferDataTyped$3$x($.gl, 34963, new Int16Array(H._ensureNativeList(indices)), 35044);
    }
  },
  Mesh$fromFile_closure0: {
    "^": "Closure:11;",
    call$1: function(e) {
      return P.print("An error has occured while loading a mesh from file!");
    }
  },
  Vertex: {
    "^": "Object;pos",
...
["vector_math", "package:vector_math/vector_math.dart", , T, {
  ...
  Matrix4: {
    ...
    $sub: function(_, arg) {
      ...
      t1 = new Float32Array(16);
      t2 = this.storage;
      t1[0] = C.JSNumber_methods.$sub(t2[0], arg.get$storage().$index(0, 0));
      t1[1] = C.JSNumber_methods.$sub(t2[1], arg.get$storage().$index(0, 1));
      t1[2] = C.JSNumber_methods.$sub(t2[2], arg.get$storage().$index(0, 2));
      t1[3] = C.JSNumber_methods.$sub(t2[3], arg.get$storage().$index(0, 3));
      t1[4] = C.JSNumber_methods.$sub(t2[4], arg.get$storage().$index(0, 4));
      t1[5] = C.JSNumber_methods.$sub(t2[5], arg.get$storage().$index(0, 5));
      t1[6] = C.JSNumber_methods.$sub(t2[6], arg.get$storage().$index(0, 6));
      t1[7] = C.JSNumber_methods.$sub(t2[7], arg.get$storage().$index(0, 7));
      t1[8] = C.JSNumber_methods.$sub(t2[8], arg.get$storage().$index(0, 8));
      t1[9] = C.JSNumber_methods.$sub(t2[9], arg.get$storage().$index(0, 9));
      t1[10] = C.JSNumber_methods.$sub(t2[10], arg.get$storage().$index(0, 10));
      t1[11] = C.JSNumber_methods.$sub(t2[11], arg.get$storage().$index(0, 11));
      t1[12] = C.JSNumber_methods.$sub(t2[12], arg.get$storage().$index(0, 12));
      t1[13] = C.JSNumber_methods.$sub(t2[13], arg.get$storage().$index(0, 13));
      t1[14] = C.JSNumber_methods.$sub(t2[14], arg.get$storage().$index(0, 14));
      t1[15] = C.JSNumber_methods.$sub(t2[15], arg.get$storage().$index(0, 15));
      return new T.Matrix4(t1);
    },
  ...
  Vector3: {
    ...
    $sub: function(_, other) {
      ...
      t3 = C.JSNumber_methods.$sub(t1[1], other.get$storage().$index(0, 1));
      t1 = C.JSNumber_methods.$sub(t1[2], other.get$storage().$index(0, 2));
  ...
  Vector4: {
    ...
    $sub: function(_, other) {
      ...
      t2 = C.JSNumber_methods.$sub(t1[0], other.get$storage().$index(0, 0));
      t3 = C.JSNumber_methods.$sub(t1[1], other.get$storage().$index(0, 1));
      t4 = C.JSNumber_methods.$sub(t1[2], other.get$storage().$index(0, 2));
      t1 = C.JSNumber_methods.$sub(t1[3], other.get$storage().$index(0, 3));
...
J.validateProgram$1$x = function(receiver, a0) {
  return J.getInterceptor$x(receiver).validateProgram$1(receiver, a0);
};
...
Isolate.$lazy($, "keys", "keys", "get$keys", function() {
  var t1 = Array(256);
  t1.fixed$length = init;
  return H.setRuntimeTypeInfo(t1, [P.bool]);
});
Isolate.$lazy($, "pkeys", "pkeys", "get$pkeys", function() {
  var t1 = Array(256);
  t1.fixed$length = init;
  return H.setRuntimeTypeInfo(t1, [P.bool]);
});
Isolate.$lazy($, "timer", "timer", "get$timer", function() {
  var t1 = Date.now();
  new P.DateTime(t1, false).DateTime$_now$0();
  return t1;
});
...
//# sourceMappingURL=webgl.dart.js.map
{
  var message =
...
function dart_precompiled($collectedClasses) {
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Mesh.prototype = $desc;
  function Mesh$fromFile_closure(this_0) {
    this.this_0 = this_0;
  }
  Mesh$fromFile_closure.builtin$cls = "Mesh$fromFile_closure";
  if (!"name" in Mesh$fromFile_closure)
    Mesh$fromFile_closure.name = "Mesh$fromFile_closure";
  $desc = $collectedClasses.Mesh$fromFile_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Mesh$fromFile_closure.prototype = $desc;
  function Mesh$fromFile_closure0() {
  }
  Mesh$fromFile_closure0.builtin$cls = "Mesh$fromFile_closure0";
  if (!"name" in Mesh$fromFile_closure0)
    Mesh$fromFile_closure0.name = "Mesh$fromFile_closure0";
  $desc = $collectedClasses.Mesh$fromFile_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Mesh$fromFile_closure0.prototype = $desc;
  function Vertex(pos) {
    this.pos = pos;
  }
